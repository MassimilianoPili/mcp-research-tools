...
public class ResearchValidationTools {
    ...
    public ResearchValidationTools(@Qualifier("researchHttpClient") WebClient httpClient) {
    ...
    public Mono<String> validatePaper(
        ...
        if (validationCache != null) {
            try {
                String cached = validationCache.get(normalizedTitle).block();
                if (cached != null) {
        ...
        return fetchSemanticScholar(encodedTitle, title)
                ...
                .doOnNext(result -> {
                    ...
                    if (validationCache != null && isSuccess) {
                        validationCache.put(normalizedTitle, result).subscribe();
                    }
    ...
    private String buildValidation(JsonNode s2, JsonNode dblp, JsonNode openAlex,
        ...
        try {
            ...
            result.put("has_corrections", !corrections.isEmpty());
    ...
    private String extractDblpVenue(JsonNode dblpInfo) {
        ...
        if (key.startsWith("journals/")) return key.split("/")[1].toUpperCase();
        else if (key.startsWith("conf/")) return key.split("/")[1].toUpperCase();
        return "";
    }

    private int parseDblpYear(JsonNode dblpInfo) {
        try { return Integer.parseInt(dblpInfo.path("year").asText("")); }
    ...
    private String validationToExtract(String validationJson, String queryTitle) {
        try {
            ...
            if (paperAuthors.isArray()) {
                for (JsonNode a : paperAuthors) authors.add(a.asText());
            }
